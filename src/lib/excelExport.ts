...
function applySheetContent(
  ws: ExcelJS.Worksheet,
  params: ExportParams,
  logoImageId: number | null,
) {
  const { records, projectName, projectAddress, workerName, calendarWeek, year, projectLocation } = params;
  ...
  // Total hours with dark background
...
export async function exportWeeklyRecordsToExcel(params: ExportParams): Promise<void> {
  ...
  applySheetContent(ws, params, logoImageId);

  // Generate filename
  const safeWorkerName = params.workerName.replace(/[^a-zA-Z0-9\s]/g, "").replace(/\s+/g, "_");
  const filename = `Leistungsnachweis_KW${params.calendarWeek}_${params.year}_${safeWorkerName}.xlsx`;
...
export async function exportMultipleWeeksToExcel(
  ...
  for (const weekData of weeks) {
    ...
    applySheetContent(ws, weekData, logoImageId);
  }
